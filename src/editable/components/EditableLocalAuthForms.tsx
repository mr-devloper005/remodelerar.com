...
const inputClass = 'h-12 border border-white/[0.06] bg-[#0f1329] px-4 text-sm font-medium text-white outline-none transition placeholder:text-white/20 focus:border-[#3d3dba]'
const buttonClass = 'inline-flex h-12 items-center justify-center bg-[#3d3dba] px-6 text-sm font-bold uppercase tracking-[0.15em] text-white transition hover:brightness-110 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-900/30 text-emerald-300' : 'bg-red-900/30 text-red-300'}`}>{message}</p> : null}
